...
Recombination must be added into `sim_pop` before the fitness computation step.
...
* Then the recombinant sequence is a recombinant between these two cluster,
...
Is a recombinant if the recombination event occurred 5 cycles ago? What about
10 cycles ago?
...
that it does not affect the other sequence. But which of the two sequences
that recombined will be replaced? **Just pick at random.**
...
* `recomb_muts`: The number of mutations introduced into the sequence due to
...
**NOTE:** For `recomb_muts`, is the above the most useful version of this
metric? Since the reference sequence is gone, it may not have any value. If
instead the definition was, how many mutations would have to be introduced
into the recombination partner (the one that stays in the dataset) to make it
identical to the recombinant, then the measure is relative to something that
still exists. The sequence that was deleted can be summarized with some
general distance to the rest of the sequences in the dataset. An important
detail that is lost in treating this a the distance to the remaining sequence
is that you no longer know in what percentage of cases the recombination event
introduced no changes. Also, since the recombinant is in the dataset and and
you have the id of the partner, you can easily recompute the number of
mutations required in the partner to make it identical to the recombinant.
...
* draw to determine the recombination partner
* call `recombine_seqs`
* update the genealogy data structure

Ignore optimizations for now. By migrating to a vector based system as for
`get_fit_offpring` large performance boosts can be obtained. Since only
updates are required, it might be really simple to implement the vector based
system. But first build a reference implementation using data.frames.
...
* packaging all the recombination goodies into either a
  - data.frame row (for the initial implementation)
  - a list (or an environment?) (for the optimized implementation)

All the recombination functionality lives in the `recombine.R` script.
...
$$D_{max} = min(p_{1}p_{2}, (1-p_{1})(1-p_{2})) if p_{12} < p_{1}p_{2}$$

and

$$D_{max} = min((1-p_{1})p_{2}, (1-p_{1})p_{2}) if p_{12} > p_{1}p_{2}$$
...
The co-occurance data is summarized in a four dimensional list:

* Dimension 1: The first position
* Dimension 2: The nucleotide at the first position
* Dimension 3: The second position
* Dimension 4: The nucleotide at the second position

The element is the number of sequences where both nucleotides corresponded
with the nucleotides in the 2nd and 4th dimensions at the positions listed in
the 1st and 3rd dimensions.
