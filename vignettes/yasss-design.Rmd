...
* `for p1 in 1:(seq_length-1)`
  - n1 <- most frequent nucleotide @ p1
    + if n1 not in (.2, .8) CONTINUE
    + `for p2 in (p1+1):seq_length`
      * n2 <- most frequent nucleotide @ p2
      * if n2 not in (.2, .8) CONTINUE
      * jot[p1, p2] += 1

* check that upper triangle is all zero

* `for p1 in 1:nrow(jot)`
  - `for p2 in (p1+1):ncol(jot)`
    + `link_dist <- p2 - p1`
    + `D = jot[p1, p2] - max_freq[p1]*max_freq[p2]`
